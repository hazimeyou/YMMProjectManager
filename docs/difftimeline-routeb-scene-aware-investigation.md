...
## Step 15: Preview Featureization & UI Integration
- investigation UI を Preview candidate UI へ再整理。
- production feature ではなく、read-only / disabled by default を維持。
- viewerWired=False 維持。
- openMode=ReadOnlyDryRun 維持。
- diagnostics は詳細表示側へ移動し、通常表示は簡潔化。
- YMM標準風の5セクション構成:
  - Current Scene
  - Related History
  - Match Reason
  - RouteA Detail Preview
  - Diagnostics / Safety

## Step 16: RouteA Viewer Wired Read-only Open Sandbox
- RouteB selected candidate can open RouteA detail viewer in read-only sandbox.
- Manual button click only.
- Safety guard requires:
  - Preview gate disabled/preview-only
  - snapshot pair resolved
