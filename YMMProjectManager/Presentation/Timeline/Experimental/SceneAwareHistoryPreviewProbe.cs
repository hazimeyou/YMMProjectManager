...
internal static partial class SceneAwareHistoryPreviewProbe
{
    ...
    public static SceneAwareHistoryPreviewProbeResult Run(string diagnosticsDirectory)
    {
        ...
        {
            HeavyProjectValidation = new RouteBHeavyProjectValidation(
                Prepared: true,
                Validated: "LightweightOnly",
                Reason: "Large-scale runtime validation pending."),
            VirtualizationFoundation = new RouteBVirtualizationFoundationStatus(
                Prepared: true,
                Enabled: false,
                Recommended: heavyProjectHeuristics.RecommendedVirtualization,
                Reason: "Foundation only; production virtualization not enabled.")
        };
    ...
    private static SceneAwarePreviewVirtualization BuildPreviewVirtualization(int totalCandidates, int displayedCandidates, bool recommended)
        => new(
            Prepared: true,
            Enabled: false,
            DefaultDisabled: true,
            ...
            DisplayedCandidateCount: displayedCandidates,
    ...
    private static RouteBFinalReadiness BuildRouteBFinalReadiness()
        => new(
            Prepared: true,
            Confidence: "High",
            CanPromoteToPreviewFeature: false,
            Reason: "Final investigation RC only; production UI, viewer wiring, and runtime mutation are not enabled.",
            CompletedCapabilities:
            [
                "runtime timeline detection",
                "runtime fingerprint",
                ...
                "heavy project heuristics",
                "performance diagnostics",
                "virtualization foundation"
            ],
            RemainingBeforeProduction:
            [
                "runtime UI validation on real heavy projects",
                "viewer wired read-only open validation",
...
- schemaVersion: {r.SceneAwareMetadata.SchemaVersion}
- createdAt: {r.SceneAwareMetadata.CreatedAt:O}
- stableHash: {r.SceneAwareMetadata.TimelineFingerprint.StableHash}
- itemCount: {r.SceneAwareMetadata.TimelineFingerprint.ItemCount}
- routeAHandoff.compareSessionId: {r.SceneAwareMetadata.RouteAHandoff.CompareSessionId ?? "(none)"}
- routeAHandoff.snapshotId: {r.SceneAwareMetadata.RouteAHandoff.SnapshotId ?? "(none)"}
...
## RouteB Final Readiness
- prepared: {r.RouteBFinalReadiness.Prepared}
...
internal sealed record RouteBPreviewFeatureReadiness(
    ...
    IReadOnlyList<string> BlockedScopes,
    RouteBHeavyProjectValidation? HeavyProjectValidation = null,
    RouteBVirtualizationFoundationStatus? VirtualizationFoundation = null);

internal sealed record RouteBHeavyProjectValidation(
    bool Prepared,
    string Validated,
    string Reason);

internal sealed record RouteBVirtualizationFoundationStatus(
    bool Prepared,
    bool Enabled,
    bool Recommended,
    string Reason);
...
internal sealed record SceneAwarePreviewVirtualization(
    bool Prepared,
    bool Enabled,
    bool DefaultDisabled,
    string Mode,
...
internal sealed record RouteBFinalReadiness(
    bool Prepared,
    string Confidence,
    bool CanPromoteToPreviewFeature,
    string Reason,
...
internal sealed record SceneAwareMetadataBlock(
    int SchemaVersion,
    DateTimeOffset CreatedAt,
    SceneAwareMetadataSource Source,
    SceneAwareMetadataSceneIdentity SceneIdentity,
    SceneAwareMetadataTimelineFingerprint TimelineFingerprint,
    SceneAwareMetadataRouteAHandoff RouteAHandoff,
    SceneAwareMetadataPrivacy Privacy);

internal sealed record SceneAwareMetadataSource(
    string Route,
    string Investigation,
    bool DefaultDisabled,
    bool ReadOnly);

internal sealed record SceneAwareMetadataSceneIdentity(
    string SceneName,
    int? SceneIndex,
    string TimelineFingerprintHash,
    string Confidence);

internal sealed record SceneAwareMetadataTimelineFingerprint(
    string StableHash,
    int ItemCount,
    int? LayerCount,
    long? MinFrame,
...
internal sealed record SceneAwareMetadataRouteAHandoff(
    string? CompareSessionId,
    string? SnapshotId,
    string? OldSnapshotId,
    string? NewSnapshotId,
...
internal sealed record SceneAwareMetadataPrivacy(
